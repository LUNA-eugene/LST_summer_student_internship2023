...
class Data_infobox(BaseModel):
    data: str
    name_rule_temple : str | None = 'isTemple01' 
    name_rule_palace : str | None =  'isPalace01'
    name_rule_nationalpark : str | None =  'isNationalPark01'
    name_rule_university: str | None =  'isUniversity01' 
    name_rule_hospital : str | None =  'isHospital01'
    name_rule_default: str | None =  'isDefault01'
...
syn_โรงพยาบาล = {'location': ['ที่ตั้ง', 'ที่อยู่' ,  'ตั้งอยู่'],
 'type': ['ประเภท', 'เป็น'],
 'number of beds': ['จำนวนเตียง', 'ขนาด'],
 'website': ['เว็บไซต์'],
 'affiliation': ['สังกัด']}
...
@app.get("/postag-ner/{data}")
async def postag_ner_endpoint(data: str):
    
    # ทำการประมวลผลด้วยฟังก์ชัน postag_ner
    ner = NER("thainer")
    data_ner = ner.tag(data)
...
@app.post("/template_infobox_place_withgooglesheet/")
async def template_infobox_place_withgooglesheet(data: Data_infobox):
    from pythainlp.util import normalize
    import re
    import pandas as pd
    import ast
    ...
    #data = normalize(data)
    data_ner_pos = postag_ner(data.data)
    df = pd.DataFrame(data_ner_pos, columns=['Word', 'NER', 'POS tags'])
    infobox = {}

    if re.search(r'เป็นวัด|เป็นพระอารามหลวง', data.data) or df['Word'][0] in ['วัด']:
        # วัด
        rule = read_googlesheet('rule_Temple')
        dict = select_rule(data.name_rule_temple, rule = rule)
    ...
    # พระราชวัง
        ...
        dict = select_rule(data.name_rule_palace, rule = rule)
    ...
    # อุทยานแห่งชาติ
        ...
        dict = select_rule(data.name_rule_nationalpark, rule = rule)
    ...
    # สถาบันอุดมศึกษา
        ...
        dict = select_rule(data.name_rule_university, rule = rule)
    ...
    # โรงพยาบาล
        ...
        dict = select_rule(data.name_rule_hospital, rule = rule)
    ...
    # default
        ...
        dict = select_rule(data.name_rule_default, rule = rule)
